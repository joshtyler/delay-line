...
\review{talk about what power is required by the FPGA here. E.g. mention how LDO powers circuitry etc.}

\review{Also MUST mention removal of \SI{100}{\ohm} resistors, because it is referenced later}
...
The heater power supply is a \SI{6.3}{\volt} \gls{rms} \SI{50}{\hertz} \gls{ac} supply. In the regeneration chassis it is used to power the heaters for each valve. A typical valve used in the design draws \SI{1}{\ampere}, so the \review{xx mA} load from the delay line will add a negligible load onto this supply.
...
\chapter{\glsentryshort{spice} Simulation}
...
\section{Delay Line Input Simulation}

\section{Delay Line Output Simulation}

\section{Store Regeneration Input Simulation}
